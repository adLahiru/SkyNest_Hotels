...
export class BookingComAPI {
     ...
     */
    async fetchReservations(fromDate?: string, toDate?: string): Promise<BookingComReservation[]> {
        try {
            const params: any = {
            ...
            const response = await this.client.get<BookingComAPIResponse>('/reservations', {
                params
            });
     ...
     */
    async getReservation(reservationId: string): Promise<BookingComReservation | null> {
        try {
            const response = await this.client.get<BookingComReservation>(
     ...
     */
    async closeAvailability(externalRoomId: string, startDate: string, endDate: string): Promise<boolean> {
        try {
     ...
     * @param endDate - End date in YYYY-MM-DD format
     */
    async openAvailability(externalRoomId: string, startDate: string, endDate: string): Promise<boolean> {
        try {
     ...
     */
    async updateInventory(externalRoomId: string, date: string, availableCount: number): Promise<boolean> {
        try {
    ...
    async batchUpdateAvailability(
        ...
        updates: { date: string; available: boolean }[]
    ): Promise<boolean> {
        try {
 ...
 */
export function createBookingComClient(config: BookingComConfig): BookingComAPI {
