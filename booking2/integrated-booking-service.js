...
export class IntegratedBookingService {
    ...
    async cancelBooking(bookingId, reason) {
        ...
        try {
            ...
            const [result] = await connection.execute('UPDATE bookings SET status = ?, cancellation_reason = ?, cancelled_at = NOW() WHERE id = ?', ['cancelled', reason || null, bookingId]);
            const affectedRows = result.affectedRows;
            if (affectedRows === 0) {
                throw new Error('Booking not found');
            }
            // 2. Get room ID to update status
            const [bookingRows] = await connection.execute('SELECT room_id FROM bookings WHERE id = ?', [bookingId]);
            if (bookingRows[0]) {
                await connection.execute('UPDATE rooms SET status = \'available\' WHERE id = ?', [bookingRows[0].room_id]);
            }
     ...
     */
    async modifyBooking(bookingId, newStartDate, newEndDate) {
        const connection = await pool.getConnection();
        try {
            await connection.beginTransaction();
            // 1. Get current booking details
            const [currentRows] = await connection.execute('SELECT room_id, start_date, end_date FROM bookings WHERE id = ?', [bookingId]);
            if (!currentRows[0]) {
                throw new Error('Booking not found');
            }
            const oldBooking = currentRows[0];
            // 2. Update booking dates
            await connection.execute('UPDATE bookings SET start_date = ?, end_date = ?, modified_at = NOW() WHERE id = ?', [newStartDate, newEndDate, bookingId]);
            await connection.commit();
 ...
 */
export function createIntegratedBookingService(bookingComConfig) {
    return new IntegratedBookingService(bookingComConfig);
}
//# sourceMappingURL=integrated-booking-service.js.map