...
async function isRoomAvailable(hotelBranchId: number, roomId: number, startDate: string, endDate: string): Promise<boolean> {
    try {
        // First verify the room exists and belongs to the specified hotel branch
        const [roomCheck] = await pool.execute<mysql.RowDataPacket[]>(
            'SELECT id FROM rooms WHERE id = ? AND hotel_branch_id = ?',
            [roomId, hotelBranchId]
        );
        ...
        const [rows] = await pool.execute<mysql.RowDataPacket[]>(
            ...
            [roomId, hotelBranchId, startDate, endDate]
        );
...
async function createBooking(bookingData: CreateBookingParams): Promise<{ bookingId: number; success: boolean; message: string }> {
    ...
    try {
        ...
        const [roomCheck] = await connection.execute<mysql.RowDataPacket[]>(
            'SELECT id, status FROM rooms WHERE id = ? AND hotel_branch_id = ?',
            [bookingData.roomId, bookingData.hotelBranchId]
        );
