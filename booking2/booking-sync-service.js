...
export class BookingSyncService {
    ...
    syncInterval = null;
    isRunning = false;
    constructor(apiConfig, syncConfig) {
        ...
        try {
            ...
            for (const reservation of reservations) {
                try {
                    ...
                    const channelBooking = this.bookingComAPI.convertToChannelBooking(reservation, this.syncConfig.channelId, roomId);
                    // Process the booking
     ...
     */
    async bookingExists(bookingReference) {
        const [rows] = await pool.execute('SELECT id FROM bookings WHERE booking_reference = ?', [bookingReference]);
     ...
     */
    async mapRoomId(bookingComRoomId) {
        // Option 1: Look up in a mapping table
        const [rows] = await pool.execute('SELECT local_room_id FROM room_mapping WHERE external_room_id = ? AND channel_type = ?', [bookingComRoomId, ChannelType.BOOKING_COM]);
 ...
 */
export function createSyncService(apiConfig, syncConfig) {
    return new BookingSyncService(apiConfig, syncConfig);
}
//# sourceMappingURL=booking-sync-service.js.map