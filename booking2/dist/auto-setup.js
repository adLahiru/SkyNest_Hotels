...
async function autoSetup() {
    ...
    try {
        ...
        const [channels] = await pool.query("SELECT id FROM booking_channels WHERE name = 'Booking.com'");
        let bookingComChannelId;
        if (channels.length === 0) {
            console.log('   Creating Booking.com channel...');
            const [result] = await pool.query("INSERT INTO booking_channels (name, enabled) VALUES ('Booking.com', true)");
            bookingComChannelId = result.insertId;
        ...
        const [rooms] = await pool.query('SELECT id, room_number, room_type FROM rooms');
        if (rooms.length === 0) {
        ...
        if (setupMappings.toLowerCase() === 'y' && rooms.length > 0) {
            for (const room of rooms) {
                const mapping = await question(`   Enter Booking.com room ID for Room ${room.id} (${room.room_number}) [or press Enter to skip]: `);
                if (mapping.trim()) {
                    try {
                        await pool.query('INSERT INTO room_mapping (external_room_id, local_room_id, channel_type) VALUES (?, ?, ?)', [mapping.trim(), room.id, 'booking_com']);
        ...
        const [mappings] = await pool.query(`SELECT rm.external_room_id, rm.local_room_id, r.room_number 
