...
export class AvailabilityPushService {
     ...
     */
    async onBookingCreated(bookingId) {
        if (!this.enabled) {
            return false;
        }
        try {
            ...
            const success = await this.bookingComAPI.closeAvailability(externalRoomId, booking.start_date, booking.end_date);
            if (success) {
    ...
    async onBookingCancelled(bookingId) {
        ...
        try {
            ...
            const success = await this.bookingComAPI.openAvailability(externalRoomId, booking.start_date, booking.end_date);
            if (success) {
     ...
     */
    async syncRoomAvailability(roomId, daysAhead = 90) {
        try {
            ...
            const [rows] = await pool.execute(`SELECT start_date, end_date 
                 FROM bookings 
                 WHERE room_id = ? 
                 AND status = 'confirmed' 
                 AND start_date >= ? 
                 AND start_date <= ?
                 ORDER BY start_date`, [roomId, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0]]);
            console.log(`[AvailabilityPush] Syncing ${rows.length} bookings for room ${roomId}`);
            // Close availability for all booked dates
            for (const booking of rows) {
                await this.bookingComAPI.closeAvailability(externalRoomId, booking.start_date, booking.end_date);
            }
            return true;
        }
        catch (error) {
    ...
    async getBookingDetails(bookingId) {
        const [rows] = await pool.execute(`SELECT b.id, b.room_id, b.start_date, b.end_date, b.status, bc.type as channel_type
             FROM bookings b
    ...
    async getExternalRoomId(localRoomId) {
        const [rows] = await pool.execute('SELECT external_room_id FROM room_mapping WHERE local_room_id = ? AND channel_type = ?', [localRoomId, ChannelType.BOOKING_COM]);
    ...
    async markBookingSynced(bookingId) {
        await pool.execute('UPDATE bookings SET synced_to_booking_com = TRUE, synced_at = NOW() WHERE id = ?', [bookingId]);
    }
    /**
    ...
    async getUnsyncedBookings() {
        const [rows] = await pool.execute(`SELECT b.id 
             ...
             LIMIT 100`, [ChannelType.BOOKING_COM]);
        return rows;
    }
}
/**
 ...
 */
export function createAvailabilityPushService(apiConfig) {
    return new AvailabilityPushService(apiConfig);
}
//# sourceMappingURL=availability-push.js.map